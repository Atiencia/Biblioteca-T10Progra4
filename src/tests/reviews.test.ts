import { describe, it, expect, vi, beforeEach, Mock } from "vitest";
import * as reviews from "../lib/reviews";
import { Review } from "../lib/models/review";
import { Vote } from "../lib/models/vote";

// --- Mocks ---
vi.mock("../lib/mongo", () => ({ connectToDatabase: vi.fn() }));
vi.mock("../lib/models/review");
vi.mock("../lib/models/vote");

const validUserId = "507f1f77bcf86cd799439011";
const validReviewId1 = "507f1f77bcf86cd799439012";
const validReviewId2 = "507f1f77bcf86cd799439013";

const mockReviews = [
  {
    _id: validReviewId1,
    user: "Alice",
    text: "Buen libro",
    rating: 5,
    bookId: "libro1",
    likes: 2,
    dislikes: 0,
    createdAt: new Date("2025-01-01T10:00:00Z").toISOString(),
  },
  {
    _id: validReviewId2,
    user: "Bob",
    text: "Interesante",
    rating: 4,
    bookId: "libro1",
    likes: 2,
    dislikes: 0,
    createdAt: new Date("2025-01-02T10:00:00Z").toISOString(),
  },
];

describe("reviews lib", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("getReviews devuelve lista vacía si no hay reseñas", async () => {
    (Review.find as unknown as Mock).mockReturnValueOnce({
      sort: () => ({ lean: () => Promise.resolve([]) }),
    });
    const result = await reviews.getReviews("bookX");
    expect(result).toEqual([]);
  });

  it("getReviews devuelve reseñas de un libro", async () => {
    (Review.find as unknown as Mock).mockReturnValueOnce({
      sort: () => ({ lean: () => Promise.resolve(mockReviews) }),
    });
    const result = await reviews.getReviews("libro1");
    expect(result).toHaveLength(2);
    expect(result[0].user).toBe("Alice");
  });

  it("addReview agrega una reseña nueva", async () => {
    const mockReview = {
      _id: validReviewId2,
      user: "Bob",
      rating: 4,
      text: "Interesante",
      bookId: "libro2",
      likes: 0,
      dislikes: 0,
      toObject: () => ({
        _id: validReviewId2,
        user: "Bob",
        rating: 4,
        text: "Interesante",
        bookId: "libro2",
        likes: 0,
        dislikes: 0,
      }),
    };
    (Review.create as unknown as Mock).mockResolvedValueOnce(mockReview);

    const review = await reviews.addReview("libro2", {
      userName: "Bob",
      rating: 4,
      text: "Interesante",
      bookTitle: "Libro 2",
      userId: validUserId,
    });

    expect(review._id).toBeDefined();
    expect(review.user).toBe("Bob");
  });

  it("voteReview aumenta likes correctamente", async () => {
    (Vote.findOne as unknown as Mock).mockResolvedValueOnce(null);
    (Review.findByIdAndUpdate as unknown as Mock).mockResolvedValueOnce({});
    (Review.findById as unknown as Mock).mockReturnValueOnce({
      lean: () => Promise.resolve({ ...mockReviews[0], likes: 3 }),
    });
    (Review.findOneAndUpdate as unknown as Mock).mockResolvedValueOnce({ ...mockReviews[0], likes: 3 });

    const updated = await reviews.voteReview(validUserId, validReviewId1, 1);
    expect((updated as { likes: number } | null)?.likes).toBe(3);
  });

  it("voteReview aumenta dislikes correctamente", async () => {
    (Vote.findOne as unknown as Mock).mockResolvedValueOnce(null);
    (Review.findByIdAndUpdate as unknown as Mock).mockResolvedValueOnce({});
    (Review.findById as unknown as Mock).mockReturnValueOnce({
      lean: () => Promise.resolve({ ...mockReviews[0], dislikes: 1 }),
    });
    (Review.findOneAndUpdate as unknown as Mock).mockResolvedValueOnce({ ...mockReviews[0], dislikes: 1 });

    const updated = await reviews.voteReview(validUserId, validReviewId1, -1);
    if (updated && !Array.isArray(updated)) {
      expect(updated.dislikes).toBe(1);
    } else {
      throw new Error("Updated review is not an object");
    }
  });
});
